...
int main() {
	...
	while (1) {
		...
		if (frameid%50==0) {
			...
			cv::goodFeaturesToTrack(present_gray, tracking_points, kMaxCount, 0.01, 10, cv::Mat(), 3, 0, 0.04);
			if (tracking_points.size() > 0) {
				cv::cornerSubPix(present_gray, tracking_points, subPixWinSize, cv::Size(-1,-1), termcrit);
			}
